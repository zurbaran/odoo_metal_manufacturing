...
class TestBlueprintFiltering(TransactionCase):
    def setUp(self):
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            ...
                            "value_ids": [(6, 0, [self.val_transp.id, self.val_carglas.id])],
                        },
                    ),
                    (
                        0,
        ...
        self.product_carglas_pulido = self._variant_for(self.val_carglas, self.val_pulido)
    ...
    def test_blueprint_is_included_when_attribute_matches(self):
        self.assertIn(
            "Plano Transparente",
            self._evaluated_names(self.product_transp_pulido, "purchase"),
        )

    def test_blueprint_is_excluded_when_attribute_does_not_match(self):
        self.assertNotIn(
            "Plano Transparente",
            self._evaluated_names(self.product_carglas_pulido, "purchase"),
        )
    ...
    def test_blueprint_with_multiple_conditions(self):
        ...
        self.assertNotIn(
            blueprint.name,
            self._evaluated_names(self.product_transp_mate, "purchase"),
        )
    ...
    def test_blueprint_with_multiple_attribute_conditions_must_all_match(self):
        ...
        self.assertIn(
            blueprint.name,
            self._evaluated_names(self.product_transp_pulido, "manufacturing"),
        )
        self.assertNotIn(
            blueprint.name,
            self._evaluated_names(self.product_transp_mate, "manufacturing"),
        )
