...
class AccountMove(models.Model):
    ...
    def create(self, vals_list):
        ...
        for move, vals in zip(moves, vals_list):
            if not vals.get('journal_id') and vals.get('company_id') and vals.get('move_type'):
                journal_type = {
                    'out_invoice': 'sale',
                    'in_invoice': 'purchase'
                }.get(vals['move_type'])
                if journal_type:
                    ...
                    if journal:
                        move.journal_id = journal
                        _logger.info("[auto_journal_by_company] CREATE: Asignado journal_id=%s (%s) para company_id=%s", journal.id, journal.name, vals['company_id'])
                    else:
