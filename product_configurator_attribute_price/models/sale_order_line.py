        ...
        for line in self:
            ...
            for custom_attribute in line.product_custom_attribute_value_ids:
                ...
                if attribute_value and attribute_value.price_formula and 'custom_value' in attribute_value.price_formula:
                    try:
                        ...
                        _logger.debug(f"[Line {line.id}] Incremento por custom_value ({attribute_value.name}): {increment}")
                    except Exception as e:
                        _logger.exception(f"[Line {line.id}] Error al evaluar la fórmula para {attribute_value.name}: {e}")
                        continue
                ...
                if no_variant_attribute and no_variant_attribute.price_formula and 'price_so_far' in no_variant_attribute.price_formula:
                    try:
                        ...
                        if increment < 0:
                            increment = 0
                        price_so_far += increment
                        _logger.debug(f"[Line {line.id}] Incremento por price_so_far ({no_variant_attribute.name}): {increment}")
                    except Exception as e:
                        _logger.exception(f"[Line {line.id}] Error al evaluar la fórmula para {no_variant_attribute.name}: {e}")
                        continue
            ...
            for custom_attribute in line.product_custom_attribute_value_ids:
                ...
                if attribute_value and attribute_value.price_extra:
                    ...
                    _logger.debug(f"[Line {line.id}] Incremento por price_extra ({attribute_value.name}): {attribute_value.price_extra}")

            for no_variant_attribute in line.product_no_variant_attribute_value_ids:
                if no_variant_attribute and no_variant_attribute.price_extra:
                    price_so_far += no_variant_attribute.price_extra
                    _logger.debug(f"[Line {line.id}] Incremento por price_extra ({no_variant_attribute.name}): {no_variant_attribute.price_extra}")
